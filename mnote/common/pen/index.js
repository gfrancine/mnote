...
(function (root, doc) {
  ...
  var autoLinkReg = {
    ...
    prefix: /^(?:https?|ftp):\/\//i,
    notLink:
      /^(?:img|a|input|audio|video|source|code|pre|script|head|title|style)$/i,
    maxLength: 100,
  };
  ...
  utils.copy = function (defaults, source) {
    utils.forEach(source, function (value, key) {
      defaults[key] = utils.is(value, "Object")
        ? utils.copy({}, value)
        : utils.is(value, "Array")
        ? utils.copy([], value)
        : value;
    });
    return defaults;
  ...
  function commandOverall(ctx, cmd, val) {
    var message = " to exec 「" + cmd + "」 command" +
      (val ? (" with value: " + val) : "");
  ...
  function initEvents(ctx) {
    ...
    if (ctx._menu) {
      ...
      outsideClick = function (e) {
        if (
          ctx._menu && !containsNode(editor, e.target) &&
    ...
    addListener(ctx, editor, "keyup", function (e) {
      ...
      if (!node || !node.nextSibling || !lineBreakReg.test(node.nodeName)) {
        return;
      }
