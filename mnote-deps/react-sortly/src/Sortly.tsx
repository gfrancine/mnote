 ...
 * @hidden
 */
const getElConnectableElement = (
  connectedDropTarget?: Connectable | React.RefObject<Connectable | undefined>
) => {
  ...
  const connectable =
    (connectedDropTarget as React.RefObject<Connectable | undefined>).current ||
    connectedDropTarget;
  ...
  const el = isRef(connectable)
    ? (connectable as React.RefObject<Element>).current
    : (connectable as Element);
  ...
  if (!horizontal) {
    ...
    if (
      (hoverClientY < hoverMiddleY &&
        isNextSibling(items, sourceIndex, targetIndex)) || // Dragging downwards
      (hoverClientY > hoverMiddleY &&
        isPrevSibling(items, sourceIndex, targetIndex)) // Dragging upwards
    ) {
      return items;
    }
  } else {
    ...
    if (
      (hoverClientX < hoverMiddleX &&
        isNextSibling(items, sourceIndex, targetIndex)) || // Dragging forwards
      (hoverClientX > hoverMiddleX &&
        isPrevSibling(items, sourceIndex, targetIndex)) // Dragging backwards
    ) {
      return items;
    }
  }
...
function Sortly<D = { id: ID }>(props: SortlyProps<D>) {
  ...
  const { dragMonitor, connectedDragSource, initialDepth } =
    React.useContext(context);
  const dndData = React.useRef<DndData>({});
  const [startAnim, stopAnim] = useAnimationFrame(
    React.useCallback(() => {
  ...
  const handleHoverBegin = React.useCallback(
    (
      id: ID,
      connectedDropTarget?: React.MutableRefObject<Connectable | undefined>
    ) => {
      dndData.current = update(dndData.current, {
        dropTargetId: { $set: id },
        connectedDropTarget: { $set: connectedDropTarget },
      });
    },
    []
  );
  ...
  const handleHoverEnd = React.useCallback((id: ID) => {
    if (dndData.current.dropTargetId === id) {
      dndData.current = update(dndData.current, {
        dropTargetId: { $set: undefined },
        connectedDropTarget: { $set: undefined },
      });
    }
  }, []);
  ...
  return (
    <sortlyContext.Provider value={{ items }}>
      {items.map((data, index) => (
        ...
        >
          <Item<D> index={index} id={data.id} depth={data.depth} data={data}>
            {children}
          </Item>
        </itemContext.Provider>
      ))}
